...
public class Frag extends Fragment implements View.OnTouchListener {
    ...
    private void init(){
        ...
        for (DB.RecordItem item : db.getItems()) {
    ...
    public void addViewItem(String id,int size, int x, int y){
        ...
        if (x == -1 && y == -1) {
            try {
                x = getNewViewPosition(ViewItem.ItemSize.min)[0];
                y = getNewViewPosition(ViewItem.ItemSize.min)[1];
            } catch (Exception e) {
        ...
        viewItem.size =  ViewItem.ItemSize.values()[size];
