...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public List<Video> getAllLocalFavorites() {  //MUST BE DONE ON DIFFERENT THREAD!!!****************$$$$$$####
        ...
        if (cursor.moveToFirst()) {
            do {
                favorites.add(new Video(cursor.getString(0),cursor.getString(1),cursor.getString(2),"wdwdwd", cursor.getString(3),cursor.getString(4),cursor.getString(5), cursor.getString(6), cursor.getString(7), cursor.getString(8)));
            } while (cursor.moveToNext());
        }
        cursor.close();
        db.close();
